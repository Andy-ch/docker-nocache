...
cd {image}
docker build -t {arch['architecture'] + arch['variant']}/{target_image}:{tag['name']} --build-arg tag={tag['name']} --build-arg arch={arch['architecture'] + arch['variant']} .
set +x
echo {os.environ['DOCKER_HUB_TOKEN']}|docker login -u andych --password-stdin
set -x
docker push {arch['architecture'] + arch['variant']}/{target_image}:{tag['name']}''',
